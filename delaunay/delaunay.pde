...
void setup(){
	...
	for(int i=0; i<10; i++) POINTS.add(new PVector(int(random(width)), int(random(height))));
}
...
void keyPressed(){
	...
	if(key=='r') setup();
	if(key==' ') POINTS.add(new PVector(random(width),random(height)));
}
void mousePressed(){ POINTS.add(new PVector(mouseX, mouseY));}
